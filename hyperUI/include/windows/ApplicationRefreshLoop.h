...
inline void run(gcroot<System::Windows::Forms::Form^> pMainForm)
{
	if(!pMainForm)
		pMainForm = gcnew IBaseForm();
	g_ptheMainForm = safe_cast<IBaseForm^>(safe_cast<System::Windows::Forms::Form^>(pMainForm));
	g_pMainWinProgram = gcnew ApplicationRefreshLoop();
	g_pMainWinProgram->Run(pMainForm);
}
/*****************************************************************************/
};
#ifdef WIN_COMPILE_UNMANAGED
