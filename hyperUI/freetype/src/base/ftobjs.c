...
#define FT_COMPONENT  trace_objs
    ...
    if ( slot )
    {
      ...
      while ( cur )
      {
        if ( cur == slot )
        {
          ...
          if ( slot->genericInfo.finalizer )
            slot->genericInfo.finalizer( slot );
  ...
  {
    /* finalize client-specific data */
    if ( size->genericInfo.finalizer )
      size->genericInfo.finalizer( size );
                ...
                FT_Driver  driver )
  {
    ...
    if ( face->genericInfo.finalizer )
      face->genericInfo.finalizer( face );
